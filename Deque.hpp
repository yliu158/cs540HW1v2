 ...
 */
 ...
 #define Deque_DEFINE(T)\
 	...
 	void Deque_##T##_sort(Deque_##T* q, const Deque_##T##_Iterator& b, const Deque_##T##_Iterator& e){\
		...
		int tmp_size;\
		if (b.index < e.index) {\
			tmp_size = e.index-b.index;\
		} else {\
			tmp_size = q->buff_size-b.index + e.index;\
		}\
		T* tmp = (T*)malloc(sizeof(T)*tmp_size);\
		for (size_t i = 0; i < tmp_size; i++) {\
			tmp[i] = q->data[(b.index+i)%q->buff_size];\
		}\
		sort(tmp, tmp+tmp_size, q->less_than);\
		for (size_t i = 0; i < tmp_size; i++) {\
			q->data[(b.index+i)%q->buff_size] = tmp[i];\
		}\
 		\
		free(tmp);\
